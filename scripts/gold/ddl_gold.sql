...
脚本用途：
  此脚本为数据仓库中的“黄金层”创建视图。
  “黄金层”表示最终维度和事实表（星型模式）。
  每个视图执行转换并合并来自“银层”的数据，以生成干净、丰富且可用于业务的数据集。
用途：
  这些视图可直接查询以进行分析和报告。
*/
...
WHERE pn.prd_end_dt IS NULL ;-- filter out all history data
...
LEFT JOIN gold.dim_customers cu ON cu.customer_key = sd.sls_cust_id
